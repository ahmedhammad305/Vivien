...
export default function User() {
  ...
  const normalizeDigits = (number) => {
    ...
    return number
      ...
      .map(
        (char) =>
          arabicToEnglishDigits[char] || hindiToEnglishDigits[char] || char
      )
      .join("");
  };
  ...
  const validatePhoneNumber = (number) => {
    ...
    if (formattedPhoneNumber.length !== 11) {
      // هنا تم تعديل 11 إلى 13 ليطابق الطلب الأصلي
  ...
  const submit = (e) => {
    ...
    axios
      .post("https://admin.vivien-cosmetics.com/api/v1/orders", {
        fullname: name,
      ...
      .then((response) => {
        ...
        if (window.fbq) {
          window.fbq("track", "Purchase", {
            ...
            product_price: unitPrice,
            currency: "EGP",
            total_cost: (Number(unitPrice) * Number(quantity) )+ Number(shipping),
          });
        }
          ...
          <div style={{ alignItems: "center" }} className="row">
            <div className="col-12 col-md-12 col-lg-6 hero" id="hero">
              <form onSubmit={submit}>
                ...
                <button type="submit" id="line" className="button btn btn-dark">
                  اضغط هنا للشراء
                </button>
              </form>
            </div>
